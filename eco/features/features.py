...
class CNNFeature(Feature):
    ...
    def get_features(self, img, pos, sample_sz, scales):
        ...
        for scale in scales:
            ...
            normalized = patch - [0.485, 0.456, 0.406]
            normalized = normalized / [0.229, 0.224, 0.225]
            normalized = normalized.transpose((2, 0, 1))
            normalized = normalized[np.newaxis, :]
            patches.append(normalized)

        if len(patches) != 1:
            patches = np.concatenate((patches[0], patches[1], patches[2], patches[3],
                                      patches[4]), axis=0)
        else:
            patches = np.array(patches[0])
        # patches初始化时(1,3,240,240)
        # patches在使用深度特征时为(5,3,240,240)
        # print(sample_sz, 'sample_size')
        # print(patches.shape,'patches')
        f1, f2 = self._forward(patches)
        # 神经网络正向输出结果
        # print(f1.shape,'f1')
        # print(f2.shape,'f2')
        # f1得到大小(60,60,64,1)或(60,60,64,5)
        # f2得到大小(15,15,512,1)或(15,15,512,5)
...
class VGG16Feature(CNNFeature):
    def __init__(self, fname, compressed_dim, session, network):
        self._ctx = mx.gpu(config.gpu_id) if config.use_gpu else mx.cpu(0)
        self.nets = network
        self.sess = session
        # self._image = tf.placeholder(tf.float32, shape=[1, None, None, 3])
        # self.init_network()
        # 现阶段在创建VGG16Feature特征类的时候进行神经网络的初始化
        # 需调整:日后需要在程序加载过程中加载神经网络,防止每次都需要加载同样的神经网络和参数
    ...
    def _forward(self, x):
        ...
        if x.shape[0] == 1:
            f1, f2 = self.nets.get_feat(self.sess, x)
        else:
            count = 0
            for i in x:
                new_x = i[np.newaxis,:,:,:]
                f11, f22 = self.nets.get_feat(self.sess, new_x)
                if count == 0:
                    f1 = f11
                    f2 = f22
                else:
                    f1 = np.concatenate((f1,f11),axis=0)
                    f2 = np.concatenate((f2,f22),axis=0)
                count += 1
        f1 = np.transpose(f1, (1, 2, 3, 0))
        f2 = np.transpose(f2, (1, 2, 3, 0))
        t2 = time.time()
        print('cost:',t2-t1)
        # f2 = np.concatenate((f2,f2),axis=2)
        return [f1, f2]
